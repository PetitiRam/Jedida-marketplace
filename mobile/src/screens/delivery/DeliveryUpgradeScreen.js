...
export default function DeliveryUpgradeScreen() {
  ...
  const requestUpgrade = async () => {
    ...
    try { await client.post('/upgrade/request', { requestedRole: 'delivery' }); await load(); }
  ...
  return (
    <ScreenContainer withHeader={false}>
      <Text style={{ fontSize: 22, fontWeight: '800', marginBottom: 8 }}>Become a delivery partner</Text>
      ...
      {!latest && <PrimaryButton title="Request to become a delivery partner" onPress={requestUpgrade} loading={busy} />}
      ...
      {latest?.status === 'approved' && <Alert type="success" message="You're approved! Open the Driver tab to register and start delivering." />}
